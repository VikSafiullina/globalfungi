...
def migrate_sh_table(db_config, original_table_name, result_table_name):
    ...
    for row in original_table_data:
        ...
        for sample, abundance in zip(samples, abundances):
            ...
            insert_query = f"INSERT INTO {result_table_name} (id, sh_name, sample_id, abundance) VALUES (%s, %s, %s, %s)"
            values = (id, sh_name, sample, abundance)
            mycursor.execute(insert_query, values)
            print(f'Inserted data for sample: {sh_name}, {sample}, abundance: {abundance}')
